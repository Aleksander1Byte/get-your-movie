...
def split_countries(s: str):
    ...
    if not isinstance(s, str):
        return []
    return [p.strip() for p in re.split(r"\s*/\s*|,|、|\||；|;|\s{2,}", s) if p and p.strip()]
...
_CN_PAT   = re.compile(r"(中国大陆|中國大陸|中国内地|中國內地|中国|china|mainland\s*china|prc|大陆)", re.I)
...
def compose_text_features(df: pd.DataFrame) -> pd.Series:
    ...
    """
    cols = [c for c in ("title","genres","directors","actors","screenwriters","languages","summary","tags") if c in df.columns]
    if not cols:
        cols = ["title"]
    return df[cols].astype(str).apply(lambda row: " | ".join(_safe_str(x) for x in row), axis=1)
...
class MovieRecommender:
    def __init__(self, df: pd.DataFrame):
        ...
        _text_fields = [
            'title','genres','tags','directors','actors','screenwriters','languages','summary'
        ]
        ...
        self.df["_norm_title"] = self.df["title"].apply(normalize_title) if "title" in self.df.columns else ""

        # ---------- Classical Feature Pipeline (always available) ----------
        char_ngram_title = TfidfVectorizer(analyzer="char", ngram_range=(2,5), min_df=2, max_df=0.99, sublinear_tf=True)
        char_ngram_summary = TfidfVectorizer(analyzer="char", ngram_range=(3,5), min_df=2, max_df=0.99, sublinear_tf=True)
        token_vec = TfidfVectorizer(token_pattern=r"[^,;/\|\s]+", lowercase=True, min_df=1, sublinear_tf=True)
        ...
        try:
            ...
            model_name = os.environ.get("GYM_SBERT_MODEL", "paraphrase-multilingual-MiniLM-L12-v2")
            self.sbert = SentenceTransformer(model_name)
            # Build embed text from several columns to give context beyond the title.
            embed_cols = [c for c in ("title","genres","tags","summary","directors","actors") if c in self.df.columns]
            if not embed_cols:
                embed_cols = ["title"]
            self.df["_embed_text"] = self.df[embed_cols].astype(str).agg(" | ".join, axis=1)
            # Encode corpus
            emb = self.sbert.encode(self.df["_embed_text"].tolist(), batch_size=64, convert_to_numpy=True, normalize_embeddings=True)
    ...
    def _match_title_to_index(self, user_title: str) -> Optional[int]:
        ...
        if exact:
            return exact[0]
        # Substring match
        contains = self.df.index[self.df["_norm_title"].str.contains(re.escape(nt), na=False)].tolist()
        if contains:
            return contains[0]
    ...
    def _encode_favorites(self, favorite_titles: List[str]) -> dict:
        ...
        if self.use_sbert and self.sbert is not None:
            try:
                neural_pref = self.sbert.encode(favorite_titles, batch_size=16, convert_to_numpy=True, normalize_embeddings=True)
    ...
    def recommend(self, favorite_titles: List[str], target_region: str, n: int = 10,
                  alpha: float = 0.85, diversify_by_year: bool = True) -> pd.DataFrame:
        """
        ...
        if target_region not in {"western","china","hongkong","japan"}:
            raise ValueError("target_region must be one of: western, china, hongkong, japan")

        # Region candidates
        mask = build_region_mask(self.df, target_region)
        candidate_idx = np.where(mask)[0]
        if len(candidate_idx) == 0:
            raise RuntimeError(f"No movies detected for region='{target_region}'. Check your 'countries' data.")
        ...
        if self.use_sbert and self.E is not None and pref_neural is not None:
            ...
            sims = (1.0 - self.NEURAL_WEIGHT) * sims_svd + self.NEURAL_WEIGHT * sims_neural
        else:
            sims = sims_svd
        ...
        ranked_idx = [candidate_idx[i] for i in order if candidate_idx[i] not in exclude]
        ...
        out_cols = [c for c in (
            "title","rating","total_ratings","genres","countries","directors","actors",
            "release_date","languages","runtime","summary","link","tags"
        ) if c in self.df.columns]

        # Transparency metrics
        idx_to_sim   = {candidate_idx[i]: float(sims[i])  for i in range(len(candidate_idx))}
        idx_to_score = {candidate_idx[i]: float(score[i]) for i in range(len(candidate_idx))}
...
def recommend_movies(favorite_titles: List[str], target_region: str, n: int = 10,
                     data_dir: str = ".", alpha: float = 0.85) -> pd.DataFrame:
...
def main():
    parser = argparse.ArgumentParser(description="Get-Your-Movie: content-based recommender (hybrid: SVD + optional neural).")
    parser.add_argument("--favorites", type=str, required=True,
                        help="Semicolon-separated list: 'In Bruges; Amélie; Un prophète'")
    parser.add_argument("--region", type=str, required=True, choices=["western","china","hongkong","japan"],
                        help="Which region to recommend from.")
    parser.add_argument("--n", type=int, default=10, help="How many movies to recommend.")
    parser.add_argument("--data_dir", type=str, default=".", help="Directory with douban_all_movies.csv")
    parser.add_argument("--alpha", type=float, default=0.85,
                        help="Blend weight: alpha*similarity + (1-alpha)*rating.")
    args = parser.parse_args()

    favs = [f.strip() for f in args.favorites.split(";") if f.strip()]
    df_out = recommend_movies(favs, args.region, n=args.n, data_dir=args.data_dir, alpha=args.alpha)

    pd.set_option("display.max_colwidth", 80)
    print("="*88)
    print(f"Recommendations ({args.region}, top {args.n}) for favorites: {', '.join(favs)}")
    print("="*88)
    cols = [c for c in ("title","rating","genres","countries","directors","actors","release_date","summary","link","similarity","score") if c in df_out.columns]
