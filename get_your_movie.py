"""
get_your_movie.py — simple content-based recommender for Douban dataset.
...
- No supervised training: TF‑IDF (char n‑grams) + cosine; optionally blend with normalized rating.

CLI:
  python get_your_movie.py --favorites "In Bruges; Amélie" --region western --n 10 --data_dir .

API:
  from get_your_movie import recommend_movies
  recs = recommend_movies(["Amélie", "Das Leben der Anderen"], "hongkong", n=10, data_dir=".")
"""
...
def detect_region_group(country_cell: str) -> Optional[str]:
    ...
    for t in tokens:
        ...
        if _CN_PAT.search(t):
            return "china"
    # If none of the Asia groups matched, but Western hints appear anywhere -> 'western'
    ...
    if col is None:
        for c in ("country", "region", "regions"):
            if c in df.columns:
                ...
                break
    if col is None:
        # No countries column — don't filter to avoid breaking functionality.
...
def compose_text_features(df: pd.DataFrame) -> pd.Series:
    ...
    Used as input for TF‑IDF vectorization.
    """
    cols = [
        c
        for c in (
...
class MovieRecommender:
    def __init__(self, df: pd.DataFrame):
        """Prepare TF‑IDF representation, normalized rating, and normalized titles for lookup."""
        self.df = df.copy()
        self.df["_text"] = compose_text_features(self.df)

        # Char n-grams (robust for CJK and typos)
        self.vectorizer = TfidfVectorizer(
            analyzer="char", ngram_range=(2, 5), min_df=2, max_df=0.95
        )
        self.X = self.vectorizer.fit_transform(self.df["_text"].fillna(""))

        # Min-max normalized rating in [0,1]; if invalid/constant -> zeros (no "magic" constants)
    ...
    def _match_title_to_index(self, user_title: str) -> Optional[int]:
        ...
        Cascade: exact match on normalized title -> substring match -> TF‑IDF similarity fallback.
        """
        ...
        if contains:
            return contains[0]
        # Fallback: TF‑IDF similarity against the full text features
        vec = self.vectorizer.transform([user_title])
        sims = self.X @ vec.T
        sims = sims.toarray().ravel() if issparse(sims) else np.asarray(sims).ravel()
        idx = int(np.argmax(sims))
        return idx if sims[idx] > 0.05 else None
    ...
    def recommend(
        ...
        - Build a user preference vector by averaging the TF‑IDF rows of favorite movies (L2-normalized).
        - Score candidates as: alpha * cosine_similarity + (1 - alpha) * normalized_rating.
        - Optionally diversify by year to avoid clusters of same-era films.
        """
        ...
        if not matched_indices:
            raise RuntimeError("None of the favorite titles matched the dataset.")

        # User profile (mean vector) + L2 normalization
        Xfav = self.X[matched_indices]
        pref = np.asarray(Xfav.mean(axis=0)).ravel()
        norm = np.linalg.norm(pref)
        if norm > 0:
            pref = pref / norm

        # Cosine similarity to candidates (CSR @ dense vector)
        Xcand = self.X[candidate_idx]
        sims = (Xcand @ pref).ravel()

        # Blend with normalized rating
        rnorm = self.df["_rating_norm"].values[candidate_idx]
        score = alpha * sims + (1.0 - alpha) * rnorm

        # Exclude the favorites themselves (just in case they also fall into candidate pool))
        ...
        # Light diversification by year (keeps variety without changing the core scoring)
        if diversify_by_year and "release_date" in self.df.columns:
            YEAR_BUCKET = 5  # size of each year bucket
            MAX_PER_BUCKET = 2  # keep up to two items per bucket
            years = []
            kept = []
            for ridx in ranked_idx:
                ...
                if y is None:
                    ...
                    if years.count(bucket) < MAX_PER_BUCKET:
                        years.append(bucket)
                        kept.append(ridx)
                if len(kept) >= n * 3:  # stop early; we only need a bit more than top-n
                    break
            ranked_idx = kept
...
def main():
    parser = argparse.ArgumentParser(
        description="Get-Your-Movie: content-based recommender for Douban datasets."
    )
    parser.add_argument(
        "--favorites",
        type=str,
